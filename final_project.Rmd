...
#
#full_listings.sf <- 
#  merge(full_listings.sf,st_drop_geometry(neighborhood_monument#s),by.x="neighbourhood_cleansed",by.y="neighbourhood")
#
nhood_rings.sf <- multipleRingBuffer(st_union(monuments.sf),3218,402) %>% plot()

nhood_dist = 
  st_buffer(st_union(monuments.sf),dist = 402) %>% st_crs(full_listings.sf)

mons = st_union(monuments.sf)

listings_dist =
  full_listings.sf %>% mutate(monumentDist = st_distance(x=full_listings.sf$geometry,y=mons$geometry))


ggplot() +
  geom_sf(data=tracts20, aes(fill=q5(distance))) +
  geom_sf(data=Metra) +
  ggtitle("Distance to Metra Stations") +
  mapTheme()
...
```{r}
smarter_model = lm(logPrice ~ log10(lagPrice)+host_response_time+host_is_superhost+neighbourhood_cleansed+bedrooms+review_scores_rating+reviews_per_month, data=st_drop_geometry(drop_na(full_listings.sf)))
