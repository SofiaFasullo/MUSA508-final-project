...
```{r}
fullOLS.lm = lm(log10(price)~.,data=st_drop_geometry(full_listings.sf))
...
fullOLS.lm2 = lm(log10(price)~.,data= full_listings.sf %>% dplyr :: select(-c(host_since, neighbourhood, monuments_nn3)) %>% st_drop_geometry() %>%  na.omit())
summary(fullOLS.lm2)
```
...
trimmedOLS.lm = lm(log10(price)~host_response_time+host_is_superhost+neighbourhood_cleansed+bedrooms+review_scores_rating+reviews_per_month, data=st_drop_geometry(full_listings.sf) %>% drop_na())
